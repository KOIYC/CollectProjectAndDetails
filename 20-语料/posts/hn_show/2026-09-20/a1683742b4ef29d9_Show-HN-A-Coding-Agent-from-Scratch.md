...
> [!meta]- 语料信息（点开展开）
> 来源：HN Show HN（post）
> 原帖：<https://news.ycombinator.com/item?id=49729456>
> 指标：点赞=2 · 评论=0 · engagement_velocity=2
> 作者：softwarewright　|　发布：2026-09-16T16:30:10Z
> 项目链接：<https://blog.softwarewrighter.com/2026/09/16/ai-tools-coding-agent-in-mlpl>
> 采集：2026-09-20T09:37:00+08:00　|　id：`a1683742b4ef29d9`
...
AI Tools #7: A Coding Agent Small Enough to Understand | Software Wrighter Lab Blog

# AI Tools #7: A Coding Agent Small Enough to Understand

September 16, 2026 • Software Wrighter

Coding agents are usually described from the outside: a product with a terminal UI, a permission system, a dozen tools, and a model behind it all. mlplcode is the inside, kept small: the whole control loop in sw-MLPL, an array language, with Rust only for the mechanisms the language cannot express and a local model for inference. One LLM primitive, six verbs, about 850 lines of MLPL that matter and 220 of Rust --- and a verifier standing between the model's claim of success and the real thing, because a 7B model will say it ran the tests when it did not.
...
mlplcode is that loop, built in the open, with the layers left off. It is the coding-agent demo in the sw-MLPL family, named in the spirit of OpenCode, and it follows one rule: MLPL owns every decision, Rust owns only the mechanisms MLPL cannot express, and a local model owns inference. The question it answers is not can MLPL call a model — it can — but can an array language express the control plane of an autonomous coding agent? The answer is yes, in about 850 lines of MLPL — 75 small functions — backed by 220 lines of Rust, and the interesting part is what those lines had to contain. (The full count, tests and all, is broken down at the end.)

| Resource | Link |
| --- | --- |
| mlplcode | sw-ml-study/demo-coding-agent |
| Read it as a book | mlplcode.org — every function, with prose · HTML · PDF |
| Worked example | the transcript of a 7B model adding a function and its test · the project it edited |
| The loop | agents/loop.mlpl · protocol.mlpl · permissions |
| sw-MLPL | sw-ml-study/sw-mlpl · playground · findings filed |
| OpenCode | anomalyco/opencode — the comparison point |
| Models | qwen2.5-coder · devstral on Ollama |
| Prior posts | Pi, the minimal agent · nono sandboxing · local-llm-loop |
| Comments | Discord |

## Why build one

Two reasons, and they are the same two reasons behind last week’s mixture-of-experts microscope.

To understand the thing by owning a small one. I use coding agents every day, and the earlier posts in this series looked at them from the outside: Pi, which is about as small as a useful agent gets; nono, which boxes one in; and local-llm-loop, which measures local models inside a plan-execute-review loop. This one is the loop itself, written so that every decision is visible as data flowing through pure functions, in a language where the whole program can be read in an afternoon. OpenCode is the architectural reference — its conceptual core is exactly the loop above — and this is explicitly not a port of it.
...
Six steps: read, write, read, write, run, done, slowed to reading pace. The model replies are replayed from the saved `qwen2.5-coder:7b` transcript; the parser, the file writes, and the test run happen for real. Recorded with VHS.
...
It wrapped file bodies in markdown fences and dropped the `END` line. It copied a concrete example path from the prompt verbatim into every action and spent the whole budget on missing-directory errors. It sent the same failing write three times in a row. Each of those got a tolerance or a guard, with the test written first: fence lines are stripped, a write to a file the run has not read is refused, an identical reply is flagged once and stops the run as `stuck` the third time, and the system prompt lost its worked example and gained ` ` placeholders. What the failures taught, in one line: a 7B model follows a text protocol only with placeholder examples, a rule against writing its own observations, and a loop that refuses unverified success.
...
The whole agent is written up as a literate document, `mlplcode.org`: every MLPL file, function by function, with the prose before each function and the function in an org-babel block that tangles back to the committed source. `just check` proves the tangle matches byte for byte, so the explanation cannot describe code that no longer exists. It opens with a ten-minute MLPL primer whose blocks you can evaluate in place, using sw-MLPL’s own org-babel backend, and it exports to HTML, PDF, text, and Markdown without evaluating anything.
...
- 项目页：[[10-项目/blog.softwarewrighter.com_657e0f8a]]
- 渠道页：[[50-渠道/hn_show]]
