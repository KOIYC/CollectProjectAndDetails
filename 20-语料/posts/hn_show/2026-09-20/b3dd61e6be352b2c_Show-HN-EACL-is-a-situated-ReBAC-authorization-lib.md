---
type: "corpus"
item_id: "b3dd61e6be352b2c"
title: "Show HN: EACL is a situated ReBAC authorization library for Datomic and Datahike"
source: "hn_show"
source_name: "HN Show HN"
url: "https://news.ycombinator.com/item?id=49712785"
project_url: "https://github.com/theronic/eacl"
author: "pgt"
published_at: "2026-09-15T14:09:39Z"
captured_at: "2026-09-20T09:37:15+08:00"
lang: "en"
kind: "post"
topic: "AI 工具/Agent"
shard: "2026-09-20"
...
> [!meta]- 语料信息（点开展开）
> 来源：HN Show HN（post）
> 原帖：<https://news.ycombinator.com/item?id=49712785>
> 指标：点赞=5 · 评论=0 · engagement_velocity=5
> 作者：pgt　|　发布：2026-09-15T14:09:39Z
> 项目链接：<https://github.com/theronic/eacl>
> 采集：2026-09-20T09:37:15+08:00　|　id：`b3dd61e6be352b2c`
...
- Homepage: http://eacl.dev/
- Default branch: main
- Created: 2019-11-23T11:16:36Z
...
EACL is a _situated_ ReBAC authorization library based on SpiceDB, built in Clojure and backed by Datomic.
...
- Authorization or **AuthZ** means "What can ` ` do?", so AuthZ is all about permissions.
...
In a ReBAC system like EACL, objects (_Subjects_ & _Resources_) are related via _Relationships_.
...
1. _Schema_ consists of `Relations` and `Permissions`:
 - `Relation` defines how a ` ` & ` ` can be related via a `Relationship`.
...
2. A _Relationship_ defines how a ` ` and ` ` are related via a named relation, e.g. `[(->user alice) :owner (->account "acme")]` means that
...
Given that an ` ` has an `:owner`, and a ` ` can have an `:account`, we can now define a permission schema that grants the `:edit` permission to all owners of the account, and the `:view` permission to all viewers of the account:
...
In EACL, **Direct Permissions** use `(Permission resource-type permission {:relation relation_name)`,
 ...
 - This means any ` ` who is an `:owner` of an ` `, will have the `update` permission for that account.
