---
type: "corpus"
item_id: "326ea3622dc1cbee"
title: "Show HN: Agent Router picks Cursor/Claude and effort per task, then launches it"
source: "hn_show"
source_name: "HN Show HN"
url: "https://news.ycombinator.com/item?id=49744490"
project_url: "https://github.com/nidhi-singh02/agent-router"
author: "nidhisinghattri"
published_at: "2026-09-17T18:13:55Z"
captured_at: "2026-09-20T14:03:02+08:00"
lang: "en"
kind: "post"
topic: "AI 工具/Agent"
shard: "2026-09-20"
...
# Show HN: Agent Router picks Cursor/Claude and effort per task, then launches it
...
> [!meta]- 语料信息（点开展开）
> 来源：HN Show HN（post）
> 原帖：<https://news.ycombinator.com/item?id=49744490>
> 指标：点赞=2 · 评论=1 · engagement_velocity=2
> 作者：nidhisinghattri　|　发布：2026-09-17T18:13:55Z
> 项目链接：<https://github.com/nidhi-singh02/agent-router>
> 采集：2026-09-20T14:03:02+08:00　|　id：`326ea3622dc1cbee`
...
- nidhi-singh02 (84 contributions)
...
| ------------------------- | ----------- | ---------------------------------------------------------------- | ------------------------ | ---------- |
| `cursor:grok-4.6` | Cursor | `agent --model cursor-grok-4.6- ` | low, medium, high | spend |
...
| `anthropic:claude-sonnet` | Claude Code | `claude --model sonnet --effort ` | low, medium, high | |
| `anthropic:claude-opus` | Claude Code | `claude --model opus --effort ` | low, medium, high | |
| `openai:gpt-6-astra` | Codex | `codex --model gpt-6-astra -c model_reasoning_effort=" "` | low, medium, high, ultra | |
...
ends with `Router session: ` and instructions for when its phase is done. With the skill
...
3. When you agree, it runs `router session ` and
 `router run --session "implement the plan in docs/plans/x.md" --dry-run`, shows you
 the card, and launches without `--dry-run` after you confirm.
4. The new session records the previous one. The card shows
 `Previous session: (planning -> implementation)`, and the new agent gets the previous
 phase and task.
...
`herdr-plugin/`. See `herdr-plugin/README.md` for the action list,
...
| `herdr agent start failed:: ` | Herdr's own error; the router closes the pane it created |
| `handoff not received by agent in pane …` | The agent is open but never started the task; paste the task there or run again |
| `agent is blocked; not resending the handoff` | The agent is waiting on a question or approval in its pane |
| `Session not found: ` | Check the id with `router session --list` |
...
See Privacy, Operations, and
Configuration for the detailed data flow and deployment guidance.
...
Agent Router is available under the MIT License.

# AX Check

## 评论（1/1）

> **nidhisinghattri** · 2026-09-17T18:15:33.000Z　
> Demo: https://youtu.be/7w8eRWnUUA8

## 关联链接

- https://github.com/nidhi-singh02/agent-router.git

## 导航

- 项目页：[[10-项目/github.com_1fbf556d]]
- 渠道页：[[50-渠道/hn_show]]
