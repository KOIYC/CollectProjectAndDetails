---
type: "corpus"
item_id: "a3af39473b9c95f7"
title: "Show HN: RelateAnything – Real-time open-vocabulary relation prediction"
source: "hn_show"
source_name: "HN Show HN"
url: "https://news.ycombinator.com/item?id=49713313"
project_url: "https://github.com/Maelic/RelateAnything"
author: "maelicn"
published_at: "2026-09-15T14:43:42Z"
captured_at: "2026-09-20T14:06:17+08:00"
lang: "en"
kind: "post"
topic: "开发者工具"
shard: "2026-09-20"
...
> [!meta]- 语料信息（点开展开）
> 来源：HN Show HN（post）
> 原帖：<https://news.ycombinator.com/item?id=49713313>
> 指标：点赞=2 · 评论=1 · engagement_velocity=2
> 作者：maelicn　|　发布：2026-09-15T14:43:42Z
> 项目链接：<https://github.com/Maelic/RelateAnything>
> 采集：2026-09-20T14:06:17+08:00　|　id：`a3af39473b9c95f7`
...
`release/make_model_cards.py`.

| model | backbone | params | A40, batch 1 | img/s, batch 32 | OVS-F1 | HICO F1 |
|---|---|---|---|---|---|---|
| `relsgg-vits16` | DINOv3 ViT-S/16 | 46.1 M | 19.5 ms | 201 | 34.6 | 36.4 |
| `relsgg-vits16plus` ⭐ | DINOv3 ViT-S/16+ | 53.2 M | 20.0 ms | 188 | 37.2 | 37.1 |
| `relsgg-vitb16` | DINOv3 ViT-B/16 | 113.8 M | 19.3 ms | 130 | 37.2 | 37.5 |
...
the five-axis composite of the headline table: that one adds A5 and
...
`training/configs/`. The baseline is
OvSGTR pre-trained on MegaSG — the
...
The protocol: `benchmark/SPEC.md`. The argument and the
traps in scene-graph metrics: docs/evaluation.md. The
...
Download: `maelic/RA-4M`.
Pipeline: `datagen/`. Format and packs: docs/data.md.
...
redistributed (AGPL upstream); `deploy/README.md` gives the
...
| what | where |
|---|---|
| train a released model | `train.sh`, the recipe in docs/training.md, the exact arguments in `training/configs/` |
| build the packs, the vocabulary, the text student | `training/` |
| evaluate on the six axes, run the OvSGTR baseline through the same scorer | `benchmark/` |
| generate RA-4M | `datagen/` |
...
Before you trust a number, read docs/pitfalls.md: every
...
| `train.py`, `train.sh` | training entry point and the released recipe |
...
| page | read it when |
|---|---|
| Installation | setting up, downloading weights and packs, running offline or on a cluster |
| Quickstart | you have a checkpoint and want triplets out of it |
| Architecture | you want to know what happens between pixels and triplets |
| Data | RA-4M, the pack format, mixtures, adding a source |
| Training | reproducing a released model or running an ablation |
| Evaluation | reporting a number or comparing against another method |
| Deployment | exporting to ONNX or OpenVINO, picking thresholds |
| Pitfalls | before you trust a number |
| The objective | adding a dataset or a loss term |
...
- **RA-4M annotations**: generated by Gemma, distributed with the
 Gemma Terms of Use notice. Images are not
 redistributed.
...
> **maelicn** · 2026-09-15T14:54:58.000Z　
> The first (and only) model for predicting any relations between any regions (boxes, masks) of an image. It runs localy on your browser or on a GPU under 20ms. This model is my biggest release so far, made entirely on my free time during the past 6 months.
> I would greatly appreciate your feedback! Please don't hesitate to try the browser demo :)

## 关联链接

- https://arxiv.org/abs/2609.12552},
- https://github.com/user-attachments/assets/76544f31-53ac-407b-9445-157e83946938
- https://maelic.github.io/RelateAnythingProject/

## 导航

- 项目页：[[10-项目/github.com_efef2c42]]
- 渠道页：[[50-渠道/hn_show]]
