---
type: "corpus"
item_id: "0634f9596e41853e"
title: "Show HN: Hermes Notes PKB"
source: "hn_show"
source_name: "HN Show HN"
url: "https://news.ycombinator.com/item?id=49121223"
project_url: "https://github.com/jpmoo/hermesnotes"
author: "jpmoo"
published_at: "2026-07-31T10:07:11Z"
...
> [!info] 一句话导读
> Self-hosted, block-first personal knowledge base: notes, tasks & projects that connect — collections (kanban/canvas/calendar), semantic search, local AI, an MCP…

> [!meta]- 语料信息（点开展开）
> 来源：HN Show HN（post）
> 原帖：<https://news.ycombinator.com/item?id=49121223>
> 指标：点赞=2 · 评论=0 · engagement_velocity=2
> 作者：jpmoo　|　发布：2026-07-31T10:07:11Z
> 项目链接：<https://github.com/jpmoo/hermesnotes>
> 采集：2026-09-21T03:11:08+08:00　|　id：`0634f9596e41853e`

## 正文

# jpmoo/hermesnotes

Self-hosted, block-first personal knowledge base: notes, tasks & projects that connect — collections (kanban/canvas/calendar), semantic search, local AI, an MCP server, and Obsidian export.
...
- *(Optional)* **Ollama** — only if you want semantic search and the AI assistant. Then pull a model, e.g. `ollama pull nomic-embed-text`.
...
**To keep it running** across reboots/crashes, use a process manager — systemd, pm2, or Docker.

---

## Configuration

Everything's optional — with nothing set, the setup wizard handles the database and generates a secret for you. If you'd rather configure by hand, copy `.env.example` to `.env` and edit it:
...
- **MCP server.** A Model Context Protocol endpoint is mounted at ` /mcp` (streamable HTTP, `Authorization: Bearer ` from Settings → Access Keys). Connect Claude or any MCP client to read/search blocks and manage tasks & projects.
...
- **Serve it over HTTPS behind a reverse proxy** (see the Caddy snippet under Install & run). Hermes trusts the proxy's `X-Forwarded-For`, so login rate-limiting and logs see the real client — not just the proxy.
...
*Only if you want to change the code* — to just run Hermes, use Install & run above.
...
Issues and pull requests are welcome. Please run `pnpm typecheck` before opening a PR. The design doc and architecture notes explain the data model and decisions.
...
- https://github.com/jpmoo/hermesnotes.git

## 导航

- 项目页：[[10-项目/github.com_d9cec3c4]]
- 渠道页：[[50-渠道/hn_show]]
