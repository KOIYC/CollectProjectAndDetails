...
> 采集：2026-09-21T02:52:47+08:00　|　id：`af412a132c2f0325`
...
> Like `--model-dir`, portable-mode files are **not** SHA-256 verified — files you placed there yourself are trusted by definition. Integrity verification applies to files textsnap *downloads*. See Security.
...
When the input is the clipboard, the recognized text is *also* placed on the clipboard — see Clipboard in, clipboard out.
...
| Flag | Description |
| --------------------- | -------------------------------------------------------------------- |
| `-o`, `--output` | Output `.txt` path. Default: `./textsnaps/ _ocr.txt`. |
...
- **SHA-256 verification.** Every downloaded file is hashed and checked against known-good digests before it's loaded. A mismatch aborts the run with a clear error rather than executing unverified weights. Digests live in `model_checksums.sha256` and are also embedded in the script as a fallback, so verification works whether you install from source or from a wheel.
- **Pinned dependencies.** `requirements-lock.txt` pins exact dependency versions for reproducible installs; the file documents how to add per-wheel `--hash` entries with `pip-compile --generate-hashes` for full supply-chain pinning.

Verification applies to files textsnap **downloads**. Model files you supply yourself — via `--model-dir` or portable mode — are trusted as-is and not re-hashed; you are responsible for their provenance.
...
Together ~890 MB. To use your own copy, either point `--model-dir` at a directory containing the same `onnx/` files plus `tokenizer.json` and `config.json`, or place those files next to the script for portable mode.
