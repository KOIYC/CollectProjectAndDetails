...
> [!meta]- 语料信息（点开展开）
> 来源：HN Show HN（post）
> 原帖：<https://news.ycombinator.com/item?id=49777057>
> 指标：点赞=2 · 评论=1 · engagement_velocity=2
> 作者：doronp　|　发布：2026-09-20T15:52:40Z
> 项目链接：<https://github.com/doronp/jevc>
> 采集：2026-09-21T09:44:03+08:00　|　id：`52c9c40b28a6ae8d`
...
- Homepage: https://www.npmjs.com/package/jev-compiler
- Default branch: main
- Created: 2026-09-18T07:53:01Z
...
- doronp (94 contributions)

---

## README

jevc compiles natural-language rules into typed questions plus a reducer in ordinary code
...
`jevc compile --lift` prints a lowering request for the agent you already have open
...
`examples/sample-project/.claude/gates/release.json`,
...
`examples/sample-project/CLAUDE.md`, line 7 — and the
...
The registration is that project's `.claude/settings.json`,
...
Code runs in. `examples/sample-project/README.md` walks
the round trip — scan, lift, compile, install, run — one command at a time.

**`docs/wiring.md`** has one recipe per surface, with the code that
actually runs:
...
`examples/GALLERY.md` is all 58 on one page, generated from
...
| `jevc compile <file\|->` | `--lift`, `--emit sdk\|json\|ai-sdk\|langchain`, `-o ` | JSON Schema → a TypeScript module (`sdk`, default), a Vercel AI SDK backend (`ai-sdk`, TypeScript), a `langchain-typesafe` classifier (`langchain`, **Python**) or a wire request (`json`); `--lift` prints the lowering request for prose. `-` reads stdin. |
| `jevc emit-policy --for <bouncer\|toolgate>` | `<program.json>`, `-o ` | Lowers a compiled program into an incumbent guardrail's own config format, after the same `validateProgram` + `lintProgram` gate `compile` runs. |
| `jevc show [fixture-id]` | `--fixtures ` | One recorded fixture end to end: the prompt it replaces, the state, the questions, the measured answers. No argument lists all 58. |
| `jevc explain ` | `--fixtures ` | Why a question exists — its provenance, the prompt it replaced, and what it measured. |
| `jevc check` | `--live`, `--fixtures ` | Replays the measured corpus offline; `--live` re-measures against the API and reports drift, one fixture at a time — a fixture that cannot be measured is one `broken` row, not a dead report. |
...
Notes; the short version is that any error-severity issue returns
...
`docs/schema-mapping.md`. `fromJsonSchema` is pure and
...
- `docs/wiring.md` — one recipe per surface, with runnable code.
- `docs/schema-mapping.md` — every JSON Schema construct, what it
 compiles to, and the exact rule behind each drop.
- `docs/validation.md` — the 25 live probes that returned 200 OK with
 a wrong answer, and what refuses each one locally.
- `docs/design.md` — the approved design spec, and the fullest argument
 for why the decomposition law is the product rather than a detail of it.
- `docs/targets/` — one file per emit target
 (bouncer,
 toolgate,
 ai-sdk and langchain,
 jev-guard, the one that was cut). These are the
...
- `fixtures/ATTRIBUTION.md` — every source whose text appears in
...
- `AGENTS.md` — the instructions for an agent working in this repository. It is
 also what `jevc scan .` finds here, which is the joke and the point.
- `CONTRIBUTING.md` — the "measured or it does not ship" standard, and
 what a change has to respect.
- `SECURITY.md` — how to report a defect privately, and what counts as one in
 a tool whose output is someone else's guardrail.
- `CHANGELOG.md` — including the breaking change in 0.1.0.
- `docs/history/implementation-plan.md` — the
...
`docs/validation.md` has the probe table with what the API actually
...
- `file` must be the one document that was lifted. A name that was never supplied cannot be
 checked, and the `// from: ` comment the emitter writes would send a reviewer
 somewhere unrelated.
...
`No answer for decision " "`. This is a **breaking change** in 0.1.0 — `ai-sdk` and
`langchain` used to return `allow` where `sdk` and `runReducer` threw, so the same Program
with the same missing

## 评论（1/1）

> **doronp** · 2026-09-20T17:45:46.000Z　
> Published to npm as well: https://www.npmjs.com/package/jev-compiler`npm i -g jev-compiler` — the binary is still `jevc`, so `jevc --help` gets you started.

## 关联链接

- https://api.typesafe.ai/v1/systemone`
- https://www.npmjs.com/package/jev-compiler

## 导航

- 项目页：[[10-项目/github.com_67049da0]]
- 渠道页：[[50-渠道/hn_show]]
