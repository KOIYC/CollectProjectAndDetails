---
type: "corpus"
item_id: "78abd3893c2521c9"
title: "Show HN: mac-screen-search: CLI find, highlight, redact text on screen, in files"
source: "hn_show"
source_name: "HN Show HN"
url: "https://news.ycombinator.com/item?id=47943666"
project_url: "https://github.com/jftuga/mac-screen-search"
author: "jftuga"
published_at: "2026-04-29T02:49:04Z"
...
> [!meta]- 语料信息（点开展开）
> 来源：HN Show HN（post）
> 原帖：<https://news.ycombinator.com/item?id=47943666>
> 指标：点赞=2 · 评论=0 · engagement_velocity=2
> 作者：jftuga　|　发布：2026-04-29T02:49:04Z
> 项目链接：<https://github.com/jftuga/mac-screen-search>
> 采集：2026-09-21T02:52:41+08:00　|　id：`78abd3893c2521c9`
...
| `-b ` | Blur matched regions with Gaussian blur (1-100); mutually exclusive with `-r` |
| `-e` | Enhanced OCR (preprocess image + check multiple candidates) |
| `-d ` | Fuzzy match using Levenshtein distance threshold |
| `-c ` | Rectangle color name (default: `red`). Available: black, blue, cyan, gray, green, magenta, orange, pink, purple, red, white, yellow |
| `-f ` | Process image files matching the glob pattern instead of capturing the screen |
| `-n` | Do not open the result in Preview (screenshot mode only) |
| `-o ` | Output directory or file path for the screenshot PNG (screenshot mode only) |
...
| `-t ` | Capture delay in seconds (default: 2; use 0 for immediate). Screenshot mode only |
| `-w` | Whole-word matching (require word boundaries on both sides of the match) |
| `-D ` | Delimiter for multiple search terms (default: `\|`). The search-term argument is split on this delimiter; each piece is searched independently and all matches are merged |
| `-T ` | Line thickness for outline rectangles (default: `4`). Accepts any positive number |
...
Use `-d ` to match strings within a Levenshtein (edit) distance of the search term. This catches OCR misreads where characters are substituted (e.g., `Z` read as `2`, `g` as `q`):
