...
> [!meta]- 语料信息（点开展开）
> 来源：HN Show HN（post）
> 原帖：<https://news.ycombinator.com/item?id=48339846>
> 指标：点赞=5 · 评论=0 · engagement_velocity=5
> 作者：dima-quant　|　发布：2026-05-30T19:36:09Z
> 项目链接：<https://github.com/dima-quant/nimic>
> 采集：2026-09-21T02:52:50+08:00　|　id：`55053e40e4cd7032`
...
- Raytracer, including a ppm-to-mp4 converter
- Python module for preprocessing image data
...
- Benchmarking performed with the raytracer demonstrates the render time for a single 512x288 scene dropped from many hours in pure Python to just 10 minutes on a single M1 CPU core.
...
frame.Y = castptr[UncheckedArray[uint8]]
frame.Cb = frame.Y
frame.Y[0] = uint8(1)
...
with var:
    stackBase = array20, ptr[uint8]
    stack = castptr[ptr[uint8]]
with let:
    indexBytes = 1024
    base = castptr[uint8]
with var:
...
castptr[ptr[uint8]].contents = p  # save pointer p to stackBase
stack <<= castptr[ptr[uint8]]  # update stack pointer
p <<= castptr[uint8]
...
p <<= castptr[uint8]
stack <<= castptr[ptr[uint8]]  # restore stack pointer

with let:
    atomStart = castptr[ptr[uint8]].contents

with let:
    xu = uint32(castintp - castintp)
    arr = castptr[UncheckedArray[uint8]]
...
- **Cast & memory** — `castT`, `sizeof(x)`, `addr(x)`, `unsafe_addr(x)`
