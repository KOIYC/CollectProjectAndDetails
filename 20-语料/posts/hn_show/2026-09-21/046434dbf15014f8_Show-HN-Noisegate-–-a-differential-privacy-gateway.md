---
type: "corpus"
item_id: "046434dbf15014f8"
title: "Show HN: Noisegate – a differential-privacy gateway for untrusted AI agents"
source: "hn_show"
source_name: "HN Show HN"
url: "https://news.ycombinator.com/item?id=49113543"
project_url: "https://github.com/yashmahajan10/llm-differential-privacy-gateway"
author: "yashmahajan10"
published_at: "2026-07-30T18:08:48Z"
...
tags:
  - 语料
  - hn_show
  - author_yashmahajan10
  - story_49113543
  - show_hn
metrics: {"points": 19, "comments": 0, "engagement_velocity": 19}
comments_count: 0
comments_total: 0
...
> [!meta]- 语料信息（点开展开）
> 来源：HN Show HN（post）
> 原帖：<https://news.ycombinator.com/item?id=49113543>
> 指标：点赞=19 · 评论=0 · engagement_velocity=19
> 作者：yashmahajan10　|　发布：2026-07-30T18:08:48Z
> 项目链接：<https://github.com/yashmahajan10/llm-differential-privacy-gateway>
> 采集：2026-09-21T03:11:14+08:00　|　id：`046434dbf15014f8`
...
- yashmahajan10 (126 contributions)
...
- **Independently verified math.** The hand-rolled noise mechanism matches OpenDP, the industry reference implementation, in all 35 noise-scale checks to within 1e-9, and 500,000-sample distribution tests pass with a positive control proving the test can fail.
- **3× the utility from better accounting.** Hybrid zCDP composition admits 308 queries where naive budget accounting stops at 100: the same privacy guarantee, three times the questions.
...
- **A small, deterministic trust boundary.** The LLM proposes queries; a boring, fully tested validation layer disposes. Nothing the model outputs can widen its own authority.
...
- **Scale agreement.** For COUNT, clamped SUM over every published numeric column, and a histogram, at five ε values (35 comparisons): our sensitivity `Δf = max(|L|, |U|)` equals OpenDP's clamp-then-sum stability bound **exactly**, and our noise scale `Δf/ε` matches the smallest scale OpenDP certifies as ε-DP to within 1e-9, in every case.
- **Distribution agreement.** 500,000 noise samples from the production sampler vs. 500,000 from OpenDP's Laplace measurement at the same scale pass a two-sample KS test (D = 0.0014, p = 0.70 at α = 0.001). A positive control proves the test has power: the same sampler, intentionally mis-scaled by 5%, is rejected at p ≈ 4×10⁻²².

OpenDP is the reference, never the implementation: it lives in the dev extras only, and the running system does not import it. The scale agreement is regression-tested in `tests/test_crosscheck_opendp.py` (skipped without OpenDP locally; CI installs it and sets `REQUIRE_OPENDP=1` so the check can never silently skip there).
...
Cumulative privacy spend against number of queries under pure, zCDP, and hybrid accounting
...
For a local (non-Docker) setup, running the tests, and configuration knobs
(budget size, epoch, identities), see `SETUP.md`.

### Use it from Claude Desktop (MCP)

The gateway also runs as an MCP stdio server, so any MCP-capable agent can ask
aggregate questions directly. The agent becomes the untrusted query author, and the
same trusted validator, engine, and budget sit below it. The querying surface exposes
only structured tools (`count`, `sum`, `average`, `histogram`, `get_budget`) whose
argument schemas are generated from the dataset policy: published columns only,
operations constrained by column kind. No `ANTHROPIC_API_KEY` is needed, because the
client agent is the intelligence.

Install the console entry point once (`pip install -e .`), then register it in
...
```json
{
  "mcpServers": {
    "noisegate": {
      "command": "/absolute/path/to/llm-dp/venv/bin/noisegate",
      "cwd": "/absolute/path/to/llm-dp",
      "env": { "DP_IDENTITY": "demo" }
    }
  }
}
```

By default it serves the bundled Adult dataset (run `python scripts/fetch_data.py`
first). Point `DP_POLICY` at an owner-authored policy YAML to serve another dataset,
`BUDGET_DB_PATH` at the persistent budget ledger (defaults to `data/budget.db`), and
`AUDIT_LOG_PATH` at the audit trail (defaults to `data/audit.log`; set it empty to
disable). Budget is keyed to identity, epoch, and dataset, so reconnecting never
refreshes it.

**Connect in four steps.** No `ANTHROPIC_API_KEY` is needed at any point: the
structured tools go straight to the trusted pipeline, and the connecting agent is the
intelligence.

1. `pip install -e .` in a virtualenv. This installs the `noisegate` entry point
 (`dp-mcp` is kept as an alias).
2. Pick a dataset. The 20-row synthetic **patients** demo ships in-repo (nothing to
 fetch); the **census** dataset needs `python scripts/fetch_data.py` once.
3. Register the server with the JSON block above. For the patients demo, add
 `"DP_POLICY": "/absolute/path/to/llm-dp/datasets/samples/patients.yaml"` to the
 `env` block. Use an absolute path, because Claude Desktop does not apply `cwd` when
 resolving env vars, so a relative path will fail.
4. Restart Claude Desktop and confirm the `noisegate` tools appear: `count`, `sum`,
 `average`, `histogram`, `get_budget`.

 Reproducible walkthrough: the patients demo, then the census at scale (click to expand)

**A reproducible session: the patients demo.** This replays the gallery's Attack 3
live, with a real agent as the attacker. Twenty synthetic patients
(`datasets/samples/patients.csv`, no PHI), among them Alice, the only woman over 64.
The policy publishes `age`, `sex`, and `diagnosis`, with ε 0.25 per query from a total
budget of 5.0. Ask these questions in order. The exact values differ every run, since the
noise is real and hiding it would misrepresent the system, but the *shape* of each
answer reproduces. Every result carries `remaining_epsilon`, so you watch the budget
deplete without ever asking for it:

1. *"How much privacy budget do I have?"* — `get_budget` reports
 `remaining ε 5.00 of 5.00` at ε 0.25 per query: enough for 20 answers, ever, for
 this identity on this dataset.
2. *"Break down the patients by diagnosis."* — a `histogram` with five bins, each a
 noisy count **± 12.0** (95% interval). Against 20 rows the noise dominates — a bin
 can even come back negative. That is the honest price of privacy at n = 20; the
 census section below shows the same guarantee costing almost nothing at n = 32,561.
 The result reports `remaining_epsilon: 4.75`.
3. *"Show the diagnosis breakdown for women aged 70 to 74."* — a disguised row lookup
 (that cohort is exactly Alice). The filter guard rejects it as a tool error: the
 4-year age window is under the required 5% of the declared 0–120 span. Rejected
 before execution — **no budget spent**.
4. *"Compare the diagnosis breakdown of all women against women 64 and under."* — the
 classic differencing pair: two broad population queries whose cohorts differ by
 exactly Alice. Both pass the guard and both are answered — and each bin carries ±12
 of noise against a ±1 signal, so subtracting the two histograms points at an
 essentially random diagnosis. The budget ticks down 0.25 for each answer
 (`remaining_epsilon: 4.25`), which is the second defense: averaging the noise away
 would take ~256 repetitions of the pair, and the budget refuses after 10.
5. *"Keep counting the patients until you are refused."* — each `count` returns
 ~20 ± 12 and `remaining_epsilon` drops another 0.25. When the budget cannot cover
 one more query, the next call returns a first-class result of
 `kind: "refusal"` — never a noisier answer, never an error to retry.
6. **Quit Claude Desktop, reopen it, ask for the budget again.** Still exhausted. The
 ledger is persisted in `BUDGET_DB_PATH` keyed to identity, epoch, and dataset;
 reconnecting never refreshes it.

**Then the census, for scale.** Remove `DP_POLICY` from the `env` block (and
restart Claude Desktop) to serve UCI Adult: n = 32,561 at ε 0.05 per query, and each
dataset keeps its own budget ledger. The same mechanism that made a 20-row cohort
unreadable barely dents a population-scale aggregate:

- *"How many people are in the dataset?"* — ≈ 32,500 **± 60**, an error of 0.2%.
- *"What is the average hours worked per week?"* — ≈ 40.5 **± 0.4**.
- *"Break the population down by education."* — 16 bins, each ± 60 on counts in the
 thousands.

That asymmetry is the point: the noise is calibrated to hide any one individual, so it
drowns questions about individuals and barely touches questions about populations. The
membership-inference chart above sweeps the same trade across ε.

**What the server guarantees.** Every tool call is answered under differential privacy
via the Laplace mechanism, or refused. There is no un-noised path and no way to retrieve
an individual row. Concretely:

- **Per-query cost.** Each answered query spends a fixed `per_query_epsilon` (0.05 on the
 bundled policy) from a finite, persisted `(ε_total, δ)` budget keyed to your identity,
 epoch, and dataset. Every result, and `get_budget`, reports the remaining budget, so
 the agent can plan the few questions that matter rather than probe.
- **Privacy unit.** The unit protected is one row on the bundled dataset (one person =
 one row); for a policy that declares an `entity` unit, it is one entity contributing at
 most *k* rows, with the noise scaled to *k*, never rows silently treated as people.
- **Refusal semantics.** When the budget cannot cover a query the server returns a
 first-class result of kind `"refusal"`, never a noisier answer and never an error to
 retry. A refusal is a policy outcome to respect; exhaustion is by design (see the
 budget-lifecycle discussion in `DESIGN.md` section 5.4).
- **Audit trail.** Below the trust boundary the service appends one record per *answered*
 release to `AUDIT_LOG_PATH`: identity, the validated query AST, the epsilon charged,
 and the remaining budget, with no raw data value and no answer figure. It records what
 actually ran and what it cost, so a deployment can reconstruct exactly how a budget was
 spent.

**What these surfaces are *not*.** Both the MCP server and the HTTP + Streamlit demo show
the trust-boundary and budget discipline; neither is a hardened production service. Each
serves a **single local principal** who owns the machine (the MCP server binds one identity
at launch, the HTTP demo reads a spoofable `X-Identity` header), and that principal can
rotate the epoch or delete the budget file to start fresh. So the "an external party burns
budget and cannot get it back" property is *demonstrated in mechanism, not enforced against
a remote adversary*; that needs the verified-identity (OAuth) and multi-tenant split still
on the roadmap. The noise sampler also has a known floating-point side channel (Mironov
2012; `DESIGN.md` section 5.5), out of scope here. **Do not** point either
surface at real regulated data (PHI, or PII under a compliance regime) or treat it as a
substitute for a vetted DP library where an untrusted party can authenticate. The realistic
scope is one-row-per-subject demo datasets and a local, trusted operator.
