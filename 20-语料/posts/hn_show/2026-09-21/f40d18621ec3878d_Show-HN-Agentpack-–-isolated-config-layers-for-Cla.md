---
type: "corpus"
item_id: "f40d18621ec3878d"
title: "Show HN: Agentpack – isolated config layers for Claude Code, Codex, and OpenCode"
source: "hn_show"
source_name: "HN Show HN"
url: "https://news.ycombinator.com/item?id=48345976"
project_url: "https://nexo.sh/posts/agentpack"
author: "nexo-v1"
published_at: "2026-05-31T14:32:55Z"
...
> [!meta]- 语料信息（点开展开）
> 来源：HN Show HN（post）
> 原帖：<https://news.ycombinator.com/item?id=48345976>
> 指标：点赞=4 · 评论=1 · engagement_velocity=4
> 作者：nexo-v1　|　发布：2026-05-31T14:32:55Z
> 项目链接：<https://nexo.sh/posts/agentpack>
> 采集：2026-09-21T02:52:44+08:00　|　id：`f40d18621ec3878d`
...
In my consulting work, of course, I don’t want to commit obscure manifest files of some unknown tool to teams’ repositories, so I create a parent -workspace/ directory structure, where I init the agentpack config in the workspace directory. Running agentpack sync will reload all the dependencies and fetch the necessary repositories from GitHub. Fetched repositories are cached in the user-wide home directory and pulled from there on repeated reads of owner/repo/path/commit to eliminate the need to fetch the same repository multiple times.
 ...
 The only state I want to keep is the manifest and lockfile; the harness-specific files should be cheap to throw away and regenerate. Using GitHub as the registry
...
List values are defined as specific selectors with the following prefixes: package: (whole package), package-path: (used when we need to have only a specific skill, MCP or agent from that package), mcp: (for toggling a specific MCP server). Then there’s conflict resolution: when both the enable and disable values contain references to the same package, we prioritize the more specialized rule (similar to how CSS cascading works). In the example above the tailwind.md rule will get prioritized, as it is a more specialized skill, and it will be available in the harness as a result.
 ...
 Collect . agentpack finds and collects all hooks.json files from four layers: the user-wide ~/./hooks.json , a plugin folder’s hooks.json , hooks specific to a SKILL.md folder, and the project-specific .agents/hooks/hooks.json . All of them are parsed into a normalized form and merged with a stable sort.
 Intermediate representation . The merged hook definitions are normalized into a typed model that has a lifecycle event ( PreToolUse , PostToolUse , …), an optional tool-name matcher (e.g. Edit , which fires only for the edit tool), and a handler ( Command , Http , Prompt , Agent ).
 Rendering per target . At this stage a harness-specific hooks configuration file is generated. For unsupported events we find the closest matching event (e.g. PermissionsRequest is modeled as PreToolUse in Cursor). Http or Agent handlers are replaced with the agentpack hook-exec command for CLIs that support only command execution.
 The runtime shim . For any emulated event, agentpack hook-exec is called. It reads args from stdin, finds the real handler, and forwards the result.
...
agentpack is a tool that grew from initially pre-defined nix home-manager configuration where I had custom aliases for Claude that included skills specific to different tech stacks by injecting a custom config through --plugin-dir argument. I rely on it every day while working with three or four agent CLIs at the same time. It’s the only package manager that stays out of my way, and it can pre-configure agents through context-aware modes. If you happen to work on different projects and use multiple coding CLIs, I think it will save you from the same mess it saved me from . Contributions, issues, and ideas are welcome on GitHub .
 References
 agentpack — the tool described in this post
 AGENTS.md — cross-tool instruction file standard
 SKILL.md convention — emerging skills format
 Model Context Protocol (MCP) — Anthropic’s announcement
 Claude Code plugins & hooks — the hook model agentpack normalizes to
 Microsoft APM — the closest alternative
#agentpack
 #ai-agents
 #claude-code
 #codex
 ...
 #developer-tools
Join the Newsletter
 Get the latest technical deep dives and startup insights.
Subscribe
 Previous
 AWS Serverless: Still the Boring Correct Choice
Back to top
 Copyright © Oleg Pustovit 2026
 |
 Privacy Policy
follow
 me
...
> **detente18** · 2026-06-02T05:48:54.000Z　
> this is pretty interesting. nice work!

## 关联链接

- https://github.com/anthropics/claude-plugins-official/blob/main/plugins/hookify/.claude-plugin/plugin.json
