---
type: "corpus"
item_id: "fdc2399a4bafd527"
title: "Show HN: Looplet – a 0-dep agent loop you own"
source: "hn_show"
source_name: "HN Show HN"
url: "https://news.ycombinator.com/item?id=47953850"
project_url: "https://github.com/hsaghir/looplet"
author: "hsaghir"
published_at: "2026-04-29T20:08:49Z"
...
> [!meta]- 语料信息（点开展开）
> 来源：HN Show HN（post）
> 原帖：<https://news.ycombinator.com/item?id=47953850>
> 指标：点赞=2 · 评论=0 · engagement_velocity=2
> 作者：hsaghir　|　发布：2026-04-29T20:08:49Z
> 项目链接：<https://github.com/hsaghir/looplet>
> 采集：2026-09-21T02:52:32+08:00　|　id：`fdc2399a4bafd527`
...
looplet pretty trace — agents from a paragraph, then run them
...
See docs/agent-factory.md for the full pattern.
...
Agents are **data**. A cartridge is a directory of files (`cartridge.json`, `config.yaml`, `prompts/system.md`, `tools/ /{tool.yaml, execute.py}`) that the loader materialises into a runnable agent. The factory builds new cartridges from English briefs; the loop engine runs them. **Zero runtime dependencies.**
...
Ship-ready hooks already wired in: `ApprovalHook`, `PermissionHook`, `CheckpointHook`, `ContextPressureHook`, `ThresholdCompactHook`, `ProvenanceSink`, `TracingHook`, `MetricsHook`, `EvalHook`, plus `DefaultCompactService` for production context management. Drop in your own in 10 lines.
...
The factory writes `cartridge.json`, `config.yaml`, `prompts/system.md`, and one `tools/ /` directory per tool the agent picks. See docs/agent-factory.md.
...
`composable_loop` is a generator — break out at any point, plug in your own hooks, swap context strategy. See docs/tutorial.md.
...
- You want zero runtime dependencies and a loop that cold-imports in ~300 ms (docs/benchmarks.md).
...
|---|---|
| `hello.cartridge` | Two-tool starter; load and run with any backend |
| `coder.cartridge` | Coding agent — bash, read, write, edit, grep, glob |
| `dep_doctor.cartridge` | Audits a repo's dependency files for security/license/maintenance risk |
...
| Doc | What's in it |
|---|---|
| **docs/agent-factory.md** | **`looplet new` — generate agents from English briefs (start here)** |
| docs/tutorial.md | Hand-write your first agent in 5 steps |
| docs/cartridge.md | Workspace file layout reference |
...
| docs/benchmarks.md | Cold-import time & dep footprint |
...
`looplet` follows SemVer. Pre-`1.0`, minor versions may introduce breaking changes — pin conservatively:
...
Bug reports, docs, backends, examples, evals all welcome. See CONTRIBUTING.md for dev setup and conventions; browse open issues labelled `good first issue` for scoped tasks. Security issues go through SECURITY.md.
...
- https://api.openai.com/v1`
- https://example.com
- https://hsaghir.com/looplet/

## 导航

- 项目页：[[10-项目/github.com_48ef6f5b]]
- 渠道页：[[50-渠道/hn_show]]
