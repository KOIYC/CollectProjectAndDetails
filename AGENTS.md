...
6. ❌ **收工不过 `kb_healthcheck.py` = 没收工**：九项不变量全绿才算完（①-⑤硬门 + ⑥⑦⑧⑨软门须逐条说明），数字对不上是改动错了。
...
| **工具** | `tools/kb_*.py` | 采集/体检/诊断/回填/订正/归档/洞察/地图/审计 | 15 个 runbook 工具（见 §2），另有自测/重渲染 |
...
**分母 =「comments_count>0 的页面」而非全部页面**——按页面数算 hn_show 只有 41%，
按真有评论的分母算 100%；v2ex/indiehackers 结构性 0，两渠道豁免）·
...
6. **渠道占比不是全网分布**：历史铺底补采只对 hn_show 可回溯（2026-04~08），
   于是存量里 hn_show 占条目 **64%**、占项目页 **74%**（超质检门槛「单渠道 ≤50%」；
   healthcheck ⑨ 按**存量口径**执法，运行日志里的「占比 15%」是当日新增口径，两者别混）。
...
1. 工具改动 → `kb_selftest.py` 全过（现 55 例）；任何改动 → `kb_healthcheck.py` 九项全绿；
   命名/框架改动 → 另跑 `kb_name_audit.py` 清零；收工前 commit（推送远端）。
2. 产出的报告/决定 → 追加 `00-索引/运行日志.md`；跨会话约定 → 更新本页与 `Home.md`。
3. 汇报格式：新语料条数 · 渠道 ok/异常/未解锁 · 渠道决定 · 回填数量 · 可用率 · 结构维护结果 · 九不变量 · 命名审计 · delta · 质检达标与否。
